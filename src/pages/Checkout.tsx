...
const Checkout = () => {
  ...
  // Redirect if no subtotal in state (user landed here improperly)
  if (!state?.subtotal) {
  ...
  const handlePaymentComplete = (paymentData: any) => {
    ...
    setTimeout(() => {
      ...
      // Clear cart after success and show tracking page after 3 seconds
      setTimeout(() => {
        clearCart();
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              {currentStep === 'tracking' && (
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                  <OrderTracking orderId={orderId} deliveryDetails={deliveryDetails} />
                </motion.div>
              )}
            </div>
