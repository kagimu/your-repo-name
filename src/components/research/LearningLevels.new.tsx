...
const getSubjectsForLevel = (level: string, curriculum: 'ugandan' | 'cambridge'): Subject[] => {
  const currData = curriculum === 'ugandan' ? UgandanCurriculum : CambridgeCurriculum;
  let subjects: Subject[] = [];

  if (level.startsWith('P') || level.includes('Grade')) {
    subjects = currData.primary.subjects.map((name, index) => ({
      id: `${name.toLowerCase()}-${level}`,
      name,
      icon: <span>{getSubjectIcon(name)}</span>,
      color: getSubjectColor(index)
    }));
  } else if (level.startsWith('S1') || level.startsWith('Year')) {
    subjects = currData.oLevel.subjects.map((name, index) => ({
      id: `${name.toLowerCase()}-${level}`,
      name,
      icon: <span>{getSubjectIcon(name)}</span>,
      color: getSubjectColor(index)
    ...
    subjects = currData.aLevel.subjects.map((name, index) => ({
      id: `${name.toLowerCase()}-${level}`,
      name,
      icon: <span>{getSubjectIcon(name)}</span>,
      color: getSubjectColor(index)
...
export const LearningLevels: React.FC<LearningLevelsProps> = ({
  ...
  const levels: Level[] = selectedCurriculum === 'ugandan' ? [
    {
      id: 'primary',
      name: 'Primary School',
      icon: <Book className="w-6 h-6" />,
      description: 'UNEB Primary Level Education',
      subjects: getSubjectsForLevel('P7', 'ugandan')
    },
    {
      id: 'o-level',
      name: 'O-Level',
      ...
      subjects: getSubjectsForLevel('S4', 'ugandan')
    },
    {
      id: 'a-level',
      name: 'A-Level',
      ...
      subjects: getSubjectsForLevel('S6', 'ugandan')
    }
  ] : [
    {
      ...
      subjects: getSubjectsForLevel('Grade 6', 'cambridge')
    },
    {
      ...
      subjects: getSubjectsForLevel('Year 11', 'cambridge')
    },
    {
      ...
      subjects: getSubjectsForLevel('A2 Level', 'cambridge')
    }
  ];
